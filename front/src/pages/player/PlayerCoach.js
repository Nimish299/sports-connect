...
const PlayerCoach = () => {
	...
	const run=async()=>{
		const response=await fetch('/api/academy/allacademys',{
			method:'GET',
			headers:{
	...
	const filterAcademys=async ()=>{
		const response =await fetch(`/api/academy/sport/${sport}`,{
			method:'GET',
			headers:{
				'Content-type':'application/json'
			}
	...
	return (
		<div>
			...
			<div>
				{academys && academys.map((acad)=>(
					<AcadListDisplay key={acad.name} academy={acad} navigate={navigate}/>
				))}
			</div>
		</div>
	)
